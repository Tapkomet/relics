...
export default class Actor5e extends Actor {
   ...
   */
  get isPolymorphed() {
    return this.getFlag("relics", "isPolymorphed") || false;
  }
  ...
  prepareDerivedData() {
    ...
    const flags = actorData.flags.relics || {};
    ...
    if (this.isPolymorphed) {
      const transformOptions = this.getFlag('relics', 'transformOptions');
      const original = game.actors?.get(this.getFlag('relics', 'originalActor'));
      if (original) {
   ...
   */
  getLevelExp(level) {
    const levels = CONFIG.RELICS.CHARACTER_EXP_LEVELS;
  ...
  getCRExp(cr) {
    ...
    return CONFIG.RELICS.CR_EXP_LEVELS[cr];
  }
  ...
  async addEmbeddedItems(items, prompt=true) {
    ...
    if (prompt) {
      const itemIdsToAdd = await SelectItemsPrompt.create(items, {
        hint: game.i18n.localize('RELICS.AddEmbeddedItemPromptHint')
      });
  ...
  static async loadClassFeatures({className="", subclassName="", level=1, priorLevel=0}={}) {
    ...
    const clsConfig = CONFIG.RELICS.classFeatures[className];
    if (!clsConfig) return [];
  ...
  _prepareSkills(actorData, bonuses, checkBonus, originalSkills) {
    ...
    // Skill modifiers
    const feats = RELICS.characterFlags;
    const athlete = flags.remarkableAthlete;
    const joat = flags.jackOfAllTrades;
  ...
  _computeSpellcastingProgression (actorData) {
    ...
    const slots = RELICS.SPELL_SLOT_TABLE[levels - 1] || [];
  ...
  _computeEncumbrance(actorData) {
    ...
    if ( game.settings.get("relics", "currencyWeight") && actorData.data.currency ) {
      const currency = actorData.data.currency;
      const numCoins = Object.values(currency).reduce((val, denom) => val += Math.max(denom, 0), 0);
      weight += numCoins / CONFIG.RELICS.encumbrance.currencyPerWeight;
    }
    ...
    if ( this.getFlag("relics", "powerfulBuild") ) mod = Math.min(mod * 2, 8);

    // Compute Encumbrance percentage
    weight = weight.toNearest(0.1);
    const max = actorData.data.abilities.str.value * CONFIG.RELICS.encumbrance.strMultiplier * mod;
    const pct = Math.clamped((weight * 100) / max, 0, 100);
    return { value: weight.toNearest(0.01), max, pct, encumbered: pct > (200/3) };
  }
  ...
  async _preCreate(data, options, user) {
    ...
    // Token size category
    const s = CONFIG.RELICS.tokenSizes[this.data.data.traits.size || "med"];
  ...
  async _preUpdate(changed, options, user) {
    ...
    if ( newSize && (newSize !== foundry.utils.getProperty(this.data, "data.traits.size")) ) {
      let size = CONFIG.RELICS.tokenSizes[newSize];
  ...
  rollSkill(skillId, options={}) {
    ...
    const reliableTalent = (skl.value >= 1 && this.getFlag("relics", "reliableTalent"));

    // Roll and return
    const rollData = foundry.utils.mergeObject(options, {
      parts: parts,
      data: data,
      title: game.i18n.format("RELICS.SkillPromptTitle", {skill: CONFIG.RELICS.skills[skillId]}),
      halflingLucky: this.getFlag("relics", "halflingLucky"),
      reliableTalent: reliableTalent,
      messageData: {
        speaker: options.speaker || ChatMessage.getSpeaker({actor: this}),
        "flags.relics.roll": {type: "skill", skillId }
      }
    });
  ...
  rollAbility(abilityId, options={}) {
    const label = CONFIG.RELICS.abilities[abilityId];
    new Dialog({
      title: game.i18n.format("RELICS.AbilityPromptTitle", {ability: label}),
      content: `<p>${game.i18n.format("RELICS.AbilityPromptText", {ability: label})}</p>`,
      buttons: {
        test: {
          label: game.i18n.localize("RELICS.ActionAbil"),
          callback: () => this.rollAbilityTest(abilityId, options)
        },
        save: {
          label: game.i18n.localize("RELICS.ActionSave"),
  ...
  rollAbilityTest(abilityId, options={}) {
    ...
    const feats = this.data.flags.relics || {};
    if ( feats.remarkableAthlete && RELICS.characterFlags.remarkableAthlete.abilities.includes(abilityId) ) {
    ...
    const rollData = foundry.utils.mergeObject(options, {
      ...
      messageData: {
        speaker: options.speaker || ChatMessage.getSpeaker({actor: this}),
        "flags.relics.roll": {type: "ability", abilityId }
      }
    });
    return d20Roll(rollData);
  }
  ...
  rollAbilitySave(abilityId, options={}) {
    ...
    const rollData = foundry.utils.mergeObject(options, {
      parts: parts,
      data: data,
      title: game.i18n.format("RELICS.SavePromptTitle", {ability: label}),
      halflingLucky: this.getFlag("relics", "halflingLucky"),
      messageData: {
        speaker: options.speaker || ChatMessage.getSpeaker({actor: this}),
        "flags.relics.roll": {type: "save", abilityId }
      }
    });
    return d20Roll(rollData);
  }
  ...
  async rollDeathSave(options={}) {
    ...
    if ( (this.data.data.attributes.hp.value > 0) || (death.failure >= 3) || (death.success >= 3)) {
      ui.notifications.warn(game.i18n.localize("RELICS.DeathSaveUnnecessary"));
      return null;
    }
    ...
    if ( this.getFlag("relics", "diamondSoul") ) {
      parts.push("@prof");
    ...
    const rollData = foundry.utils.mergeObject(options, {
      parts: parts,
      data: data,
      title: game.i18n.localize("RELICS.DeathSavingThrow"),
      halflingLucky: this.getFlag("relics", "halflingLucky"),
      targetValue: 10,
      messageData: {
        speaker: options.speaker || ChatMessage.getSpeaker({actor: this}),
        "flags.relics.roll": {type: "death"}
      }
    });
    ...
    if ( success ) {
      ...
      if ( d20 === 20 ) {
        ...
        chatString = "RELICS.DeathSaveCriticalSuccess";
      }
      ...
      // 3 Successes = survive and reset checks
        ...
        chatString = "RELICS.DeathSaveSuccess";
      }
    ...
    // Save failure
      ...
      if ( failures >= 3 ) {  // 3 Failures = death
        chatString = "RELICS.DeathSaveFailure";
      }
    }
  ...
  async rollHitDie(denomination, {dialog=true}={}) {
    ...
    if ( !cls ) {
      ui.notifications.error(game.i18n.format("RELICS.HitDiceWarn", {name: this.name, formula: denomination}));
      return null;
    }

    // Prepare roll data
    const parts = [`1${denomination}`, "@abilities.con.mod"];
    const title = game.i18n.localize("RELICS.HitDiceRoll");
  ...
  async _displayRestResultMessage(result, longRest=false) {
    ...
    switch (game.settings.get("relics", "restVariant")) {
      case 'normal': restFlavor = (longRest && newDay) ? "RELICS.LongRestOvernight" : `RELICS.${length}RestNormal`; break;
      case 'gritty': restFlavor = (!longRest && newDay) ? "RELICS.ShortRestOvernight" : `RELICS.${length}RestGritty`; break;
      case 'epic':  restFlavor = `RELICS.${length}RestEpic`; break;
    }

    // Determine the chat message to display
    if ( diceRestored && healthRestored ) message = `RELICS.${length}RestResult`;
    else if ( longRest && !diceRestored && healthRestored ) message = "RELICS.LongRestResultHitPoints";
    else if ( longRest && diceRestored && !healthRestored ) message = "RELICS.LongRestResultHitDice";
    else message = `RELICS.${length}RestResultShort`;
  ...
  convertCurrency() {
    ...
    const convert = CONFIG.RELICS.currencyConversion;
  ...
  async transformInto(target, { keepPhysical=false, keepMental=false, keepSaves=false, keepSkills=false,
    ...
    const allowed = game.settings.get("relics", "allowPolymorphing");
    if ( !allowed && !game.user.isGM ) {
      return ui.notifications.warn(game.i18n.localize("RELICS.PolymorphWarn"));
    }

    // Get the original Actor data and the new source data
    const o = this.toJSON();
    o.flags.relics = o.flags.relics || {};
    o.flags.relics.transformOptions = {mergeSkills, mergeSaves};
    ...
    d.data.attributes.stress = o.data.attributes.stress; // Keep your prior stress level
    d.data.attributes.maxStress = o.data.attributes.maxStress; // Keep your prior maximum stress level
    ...
    if (!keepClass && d.data.details.cr) {
      d.items.push({
        type: 'class',
        name: game.i18n.localize('RELICS.PolymorphTmpClass'),
    ...
    // Set new data flags
    if ( !this.isPolymorphed || !d.flags.relics.originalActor ) d.flags.relics.originalActor = this.id;
    d.flags.relics.isPolymorphed = true;
    ...
    Hooks.callAll('relics.transformActor', this, target, d, {
  ...
  async revertOriginalForm() {
    ...
    if ( !this.isOwner ) {
      return ui.notifications.warn(game.i18n.localize("RELICS.PolymorphRevertWarn"));
    }
  ...
  static addDirectoryContextOptions(html, entryOptions) {
    entryOptions.push({
      name: 'RELICS.PolymorphRestoreTransformation',
  ...
  static formatCreatureType(typeData) {
    ...
    if ( typeData.value === "custom" ) {
      ...
      let code = CONFIG.RELICS.creatureTypes[typeData.value];
      localizedType = game.i18n.localize(!!typeData.swarm ? `${code}Pl` : code);
    }
    let type = localizedType;
    if ( !!typeData.swarm ) {
      type = game.i18n.format('RELICS.CreatureSwarmPhrase', {
        size: game.i18n.localize(CONFIG.RELICS.actorSizes[typeData.swarm]),
        type: localizedType
      });
    }
