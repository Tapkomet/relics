...
export const migrateWorld = async function() {
  ui.notifications.info(`Applying Relics System Migration for version ${game.system.data.version}. Please be patient and do not close your game or shut down your server.`, {permanent: true});
  ...
  for ( let a of game.actors.contents ) {
    try {
      ...
      err.message = `Failed relics system migration for Actor ${a.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  for ( let i of game.items.contents ) {
    try {
      ...
      err.message = `Failed relics system migration for Item ${i.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  for ( let s of game.scenes.contents ) {
    try {
      ...
      err.message = `Failed relics system migration for Scene ${s.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  game.settings.set("relics", "systemMigrationVersion", game.system.data.version);
  ui.notifications.info(`Relics System Migration to version ${game.system.data.version} completed!`, {permanent: true});
};
...
export const migrateCompendium = async function(pack) {
  ...
  for ( let doc of documents ) {
    ...
    // Handle migration failures
      ...
      err.message = `Failed relics system migration for entity ${doc.name} in pack ${pack.collection}: ${err.message}`;
      console.error(err);
    }
  }
...
function cleanActorData(actorData) {
  ...
  // Scrub system flags
  const allowedFlags = CONFIG.RELICS.allowedActorFlags.reduce((obj, f) => {
    obj[f] = null;
    return obj;
  }, {});
  if ( actorData.flags.relics ) {
    actorData.flags.relics = filterObject(actorData.flags.relics, allowedFlags);
  }
...
function _migrateActorSenses(actor, updateData) {
  ...
  for ( let s of original.split(",") ) {
    ...
    if ( type in CONFIG.RELICS.senses ) {
...
function _migrateActorType(actor, updateData) {
  ...
  if ( match ) {
    ...
    const typeMatch = Object.entries(CONFIG.RELICS.creatureTypes).find(([k, v]) => {
      return (typeLc === k) ||
    ...
    // Match a swarm
    const isNamedSwarm = actor.name.startsWith(game.i18n.localize("RELICS.CreatureSwarm"));
    if ( match.groups.size || isNamedSwarm ) {
      const sizeLc = match.groups.size ? match.groups.size.trim().toLowerCase() : "tiny";
      const sizeMatch = Object.entries(CONFIG.RELICS.actorSizes).find(([k, v]) => {
...
function _migrateItemAttunement(item, updateData) {
  ...
  updateData["data.attunement"] = CONFIG.RELICS.attunementTypes.NONE;
...
export async function purgeFlags(pack) {
  const cleanFlags = (flags) => {
    const flags5e = flags.relics || null;
    return flags5e ? {relics: flags5e} : {};
  };
  await pack.configure({locked: false});
