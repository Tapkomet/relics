...
export default class ShortRestDialog extends Dialog {
  ...
  /** @override */
	static get defaultOptions() {
	  return mergeObject(super.defaultOptions, {
	    template: "systems/relics/templates/apps/short-rest.html",
      classes: ["relics", "dialog"]
    });
  }
  ...
  getData() {
    ...
    // Determine rest type
    const variant = game.settings.get("relics", "restVariant");
  ...
  static async shortRestDialog({actor}={}) {
    return new Promise((resolve, reject) => {
      const dlg = new this(actor, {
        title: game.i18n.localize("RELICS.ShortRest"),
        buttons: {
          rest: {
            icon: '<i class="fas fa-bed"></i>',
            label: game.i18n.localize("RELICS.Rest"),
            callback: html => {
              let newDay = false;
              if (game.settings.get("relics", "restVariant") === "gritty")
