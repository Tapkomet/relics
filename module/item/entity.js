...
export default class Item5e extends Item {
  ...
  get hasAreaTarget() {
    const target = this.data.data.target;
    return target && (target.type in CONFIG.RELICS.areaTargetTypes);
  }
  ...
  prepareDerivedData() {
    ...
    // Feat Items
      ...
      if ( act && (act.type === C.abilityActivationTypes.legendary) ) labels.featType = game.i18n.localize("RELICS.LegendaryActionLabel");
      else if ( act && (act.type === C.abilityActivationTypes.lair) ) labels.featType = game.i18n.localize("RELICS.LairActionLabel");
      else if ( act && act.type ) labels.featType = game.i18n.localize(data.damage.length ? "RELICS.Attack" : "RELICS.Action");
      else labels.featType = game.i18n.localize("RELICS.Passive");
    }

    // Equipment Items
    else if ( itemData.type === "equipment" ) {
      labels.armor = data.armor.value ? `${data.armor.value} ${game.i18n.localize("RELICS.AC")}` : "";
      labels.dr = data.dr.value ? `${data.dr.value} ${game.i18n.localize("RELICS.DR")}` : "";
    }
    ...
    if ( data.hasOwnProperty("activation") ) {
      ...
      labels.recharge = `${game.i18n.localize("RELICS.Recharge")} [${chg.value}${parseInt(chg.value) < 6 ? "+" : ""}]`;
    }
  ...
  getSaveDC() {
    ...
    // Update labels
    const abl = CONFIG.RELICS.abilities[save.ability];
    this.labels.save = game.i18n.format("RELICS.SaveDC", {dc: save.dc || "", ability: abl});
    return save.dc;
  }
  ...
  async roll({configureDialog=true, rollMode, createMessage=true}={}) {
    ...
    const requireSpellSlot = isSpell && (id.level > 0) && CONFIG.RELICS.spellUpcastModes.includes(id.preparation.mode);
    ...
    if ( createMeasuredTemplate ) {
      const template = game.relics.canvas.AbilityTemplate.fromItem(item);
  ...
  _getUsageUpdates({consumeQuantity, consumeRecharge, consumeResource, consumeSpellLevel, consumeUsage}) {
    ...
    if ( consumeRecharge ) {
      ...
      if ( recharge.charged === false ) {
        ui.notifications.warn(game.i18n.format("RELICS.ItemNoUses", {name: this.name}));
        return false;
      }
    ...
    if ( consumeSpellLevel ) {
      ...
      if ( spells === 0 ) {
        const label = game.i18n.localize(consumeSpellLevel === "pact" ? "RELICS.SpellProgPact" : `RELICS.SpellLevel${id.level}`);
        ui.notifications.warn(game.i18n.format("RELICS.SpellCastNoSlots", {name: this.name, level: label}));
        return false;
      }
  ...
  _handleConsumeResource(itemUpdates, actorUpdates, resourceUpdates) {
    ...
    // No consumed target
    const typeLabel = CONFIG.RELICS.abilityConsumptionTypes[consume.type];
    if ( !consume.target ) {
      ui.notifications.warn(game.i18n.format("RELICS.ConsumeWarningNoResource", {name: this.name, type: typeLabel}));
      return false;
    }
    ...
    if ( !resource ) {
      ui.notifications.warn(game.i18n.format("RELICS.ConsumeWarningNoSource", {name: this.name, type: typeLabel}));
      return false;
    }
    ...
    if ( remaining < 0 ) {
      ui.notifications.warn(game.i18n.format("RELICS.ConsumeWarningNoQuantity", {name: this.name, type: typeLabel}));
      return false;
    }
  ...
  async displayCard({rollMode, createMessage=true}={}) {
    ...
    const html = await renderTemplate("systems/relics/templates/chat/item-card.html", templateData);
    ...
    if ( (this.data.type === "consumable") && !this.actor.items.has(this.id) ) {
      chatData.flags["relics.itemData"] = this.data;
    }
  ...
  getChatData(htmlOptions={}) {
    ...
    if ( data.hasOwnProperty("equipped") && !["loot", "tool"].includes(this.data.type) ) {
      if ( data.attunement === CONFIG.RELICS.attunementTypes.REQUIRED ) props.push(game.i18n.localize(CONFIG.RELICS.attunements[CONFIG.RELICS.attunementTypes.REQUIRED]));
      props.push(
        game.i18n.localize(data.equipped ? "RELICS.Equipped" : "RELICS.Unequipped"),
        game.i18n.localize(data.proficient ? "RELICS.Proficient" : "RELICS.NotProficient"),
      );
    }
  ...
  _equipmentChatData(data, labels, props) {
    props.push(
      CONFIG.RELICS.equipmentTypes[data.armor.type],
      labels.armor || null,
      labels.dr || null,
      data.stealth.value ? game.i18n.localize("RELICS.StealthDisadvantage") : null
    );
  }
  ...
  _weaponChatData(data, labels, props) {
    props.push(
      CONFIG.RELICS.weaponTypes[data.weaponType],
    );
  }
  ...
  _consumableChatData(data, labels, props) {
    props.push(
      CONFIG.RELICS.consumableTypes[data.consumableType],
      data.uses.value + "/" + data.uses.max + " " + game.i18n.localize("RELICS.Charges")
    );
  ...
  _toolChatData(data, labels, props) {
    props.push(
      CONFIG.RELICS.abilities[data.ability] || null,
      CONFIG.RELICS.proficiencyLevels[data.proficient || 0]
    );
  }
  ...
  _lootChatData(data, labels, props) {
    props.push(
      game.i18n.localize("RELICS.ItemTypeLoot"),
      data.weight ? data.weight + " " + game.i18n.localize("RELICS.AbbreviationLbs") : null
    );
  }
  ...
  async rollAttack(options={}) {
    const itemData = this.data.data;
    const flags = this.actor.data.flags.relics || {};
    if ( !this.hasAttack ) {
      throw new Error("You may not place an Attack Roll with this Item.");
    }
    let title = `${this.name} - ${game.i18n.localize("RELICS.AttackRoll")}`;
    ...
    const rollConfig = mergeObject({
      ...
      messageData: {"flags.relics.roll": {type: "attack", itemId: this.id }}
    }, options);
  ...
  rollDamage({critical=false, event=null, spellLevel=null, versatile=false, options={}}={}) {
    ...
    const itemData = this.data.data;
    const actorData = this.actor.data.data;
    const messageData = {"flags.relics.roll": {type: "damage", itemId: this.id }};
    ...
    const actionFlavor = game.i18n.localize(itemData.actionType === "heal" ? "RELICS.Healing" : "RELICS.DamageRoll");
    ...
    if ( versatile && itemData.damage.versatile ) {
      ...
      messageData["flags.relics.roll"].versatile = true;
    }
    ...
    if ( itemData.actionType === "mwak" ) {
      rollConfig.criticalBonusDice = this.actor.getFlag("relics", "meleeCriticalDamageDice") ?? 0;
    }
  ...
  async rollFormula(options={}) {
    ...
    const title = `${this.name} - ${game.i18n.localize("RELICS.OtherFormula")}`;
    ...
    roll.toMessage({
      ...
      messageData: {"flags.relics.roll": {type: "other", itemId: this.id }}
    });
    return roll;
  }
  ...
  async rollRecharge() {
    ...
    const promises = [roll.toMessage({
      flavor: `${game.i18n.format("RELICS.ItemRechargeCheck", {name: this.name})} - ${game.i18n.localize(success ? "RELICS.ItemRechargeSuccess" : "RELICS.ItemRechargeFailure")}`,
  ...
  rollToolCheck(options={}) {
    ...
    const title = `${this.name} - ${game.i18n.localize("RELICS.ToolCheck")}`;
    ...
    const rollConfig = mergeObject({
      ...
      chooseModifier: true,
      halflingLucky: this.actor.getFlag("relics", "halflingLucky" ) || false,
      reliableTalent: (this.data.data.proficient >= 1) && this.actor.getFlag("relics", "reliableTalent"),
      messageData: {"flags.relics.roll": {type: "tool", itemId: this.id }}
    }, options);
    rollConfig.event = options.event;
  ...
  static async _onChatCardAction(event) {
    ...
    const storedData = message.getFlag("relics", "itemData");
  ...
  static _getChatCardTargets(card) {
    ...
    if ( !targets.length ) ui.notifications.warn(game.i18n.localize("RELICS.ActionWarningNoToken"));
    return targets;
  }
  ...
  _onCreateOwnedEquipment(data, actorData, isNPC) {
    ...
    if ( foundry.utils.getProperty(data, "data.proficient") === undefined ) {
      if ( isNPC ) {
        ...
        const armorProf = CONFIG.RELICS.armorProficienciesMap[data.data?.armor?.type]; // Player characters check proficiency
  ...
  _onCreateOwnedWeapon(data, actorData, isNPC) {
    ...
    if ( foundry.utils.getProperty(data, "data.proficient") === undefined ) {
      if ( isNPC ) {
        ...
        const weaponProf = CONFIG.RELICS.weaponProficienciesMap[data.data?.weaponType]; // Player characters check proficiency
  ...
  static async createScrollFromSpell(spell) {
    ...
    // Get scroll data
    const scrollUuid = `Compendium.${CONFIG.RELICS.sourcePacks.ITEMS}.${CONFIG.RELICS.spellScrollIds[level]}`;
    ...
    const spellScrollData = foundry.utils.mergeObject(scrollData, {
      name: `${game.i18n.localize("RELICS.SpellScroll")}: ${itemData.name}`,
      img: itemData.img,
      data: {
